...
class PostListView (ListView):
    ...
    def get_queryset(self):
        return Post.objects.filter(Q(published_date__lte=timezone.now()) & Q(hidden = False )).order_by('-published_date')


class PostDetailView (FormMixin,DetailView):
    model=Post
    form_class= CommentForm
    template_name = 'nsig_blog/post_detail.html'

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['publishercheck'] = False
        if str(self.request.user.username) == str(context['post'].author):
            context['publishercheck'] = True
        context['commentercheck'] = str(self.request.user.username)
...
class PostDraftView (LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        if self.request.user.is_superuser:
            return Post.objects.filter((Q(author = self.request.user ) & Q(published_date__isnull=True))| Q(hidden = True )).order_by('create_date')
        else:
            return Post.objects.filter((Q(author = self.request.user ) & Q(published_date__isnull=True))| (Q(author = self.request.user ) & Q(hidden = True ))).order_by('create_date')
...
#     return redirect('post_detail',pk=post.pk)
    # else:
        # form=CommentForm()
    # return render(request,'nsig_blog/comment_form.html',{'form':form})
